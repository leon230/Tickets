...
//Is called by the main application to give a reference back to itself.
    ...
    public void setMainApp(MainApp mainApp) {
        ...
        masterdata.addAll(mainApp.getTicketData("SELECT ID, TICKET_NO, TICKET_TITLE, TICKET_OWNER, OPEN_DATE," +
                ...
                "STATUS,INSERT_DATE,INSERT_USER FROM  tickets ORDER BY ID DESC"));
        ...
        if (fileFrom != null) {
            try {
                ...
                if (success || fileExists) {
                    ...
                    File fileTo = new File( strDirectoy + "/" + pathTag + fileFrom.getName());
        ...
        if (tempTicket != null) {
            ...
            if (okClicked) {
                try{
                    Connection c ;
                    c = DBConnect.connect();
                    String SQL = "INSERT INTO tickets (TICKET_TITLE,TICKET_OWNER, DESCRIPTION,IMPACT_ALL) VALUES ('" + tempTicket.getticketTitle() + "','" + tempTicket.getticketOwner() +
                            "','" + tempTicket.getticketDesc() + "','" + tempTicket.getticketImpactAll() +  "')";
...
//Copy files from attachment string----------------------------------------------
                        ...
                        try {
                            ...
                            for (int i = 0; i < tempTicket.getticketAttachments().length(); i++) {
                                ...
                                if (ch == ',' | i == tempTicket.getticketAttachments().length() - 1) {
                                    ...
                                    handleCopyFile(fileToCopy,"./files/" + tempTicket.getticketNO(), locDate.format(DATE_FORMATTER) + "_");
                                    fileToCopy = "";
                                }
                            }
                        } catch (Exception exc) {
                ...
                catch(Exception exc){System.out.println("DB error insert: " + exc.toString());}
                ...
                protected void updateItem(String item, boolean empty) {
                    ...
                    if (item == null || empty) {
                        ...
                        if (item.contentEquals("IN PROGRESS") ) {
                            setTextFill(Color.BLACK);
                            setStyle("-fx-background-color: #ffd480");
                        } else if (item.contentEquals("AWAITING")){
                            setTextFill(Color.BLACK);
                            setStyle("-fx-background-color: #ff4d4d");
                        }
